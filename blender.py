...
if __name__ == "__main__":
    ...
    parser.add_argument('-n','--name', type=str, required=True)# Parse the argument
    args = parser.parse_args()# Print "Hello" + the user input argument
    argpacket = json.loads(args.name)
    composting = [{"outfilebasename": argpacket["outfilebasename"], "borders_x": [float(argpacket["borderx1"]), float(argpacket["borderx2"])], "borders_y": [float(argpacket["bordery1"]), float(argpacket["bordery2"])]}]
    finalized_packet = {
