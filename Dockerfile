...
RUN git clone https://github.com/golemfactory/hash-cracker.git
RUN apt-get update -q \
...
RUN yagna service run &
RUN sleep 60;yagna app-key create requestor
RUN yagna payment init --sender
RUN apt upgrade
RUN cd hash-cracker
CMD ["python3", "requestor.py"]
