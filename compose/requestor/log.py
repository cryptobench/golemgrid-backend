...
from logutils import get_local_timezone, get_logger
...
def _check_event_type_to_string():
    ...
    _event_types_superclasses = {
        ty for ev in _event_types for ty in ev.mro() if ty is not ev}
...
class SummaryLogger:
    ...
    def _handle(self, event: events.Event):
        if isinstance(event, events.ComputationStarted):
            ...
            self.logger.info(
                f"MIN LOG: {provider_info.name}, {event.task_data}, {event.job_id}")
            self.logger.info(
            ...
            if not event.exc_info:
                ...
                self.logger.info(
                    f"Computation finished in {total_time:.1f}s", job_id=job_id)
                self.finished = True
            else:
                _exc_type, exc, _tb = event.exc_info
                if isinstance(exc, CancelledError):
                    ...
                    reason = str(exc) or repr(exc) or "unexpected error"
                    self.logger.error(
                        "Computation failed, reason: %s", reason, job_id=job_id)
